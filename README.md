...
NFT collection floor price, volume, holders, rarity data. Ethereum and Base via Reservoir. Pay-per-call via [x402](https://x402.org) (USDC on Base L2) -- no API key, no signup, no rate-limit wall.
...
|---|---|---|---|---|
| `nft_get_collection_data` | GET | `/api/collection` | $0.005 | Get NFT collection floor price, volume, and stats |
| `nft_get_token_rarity` | GET | `/api/rarity` | $0.003 | Get NFT token rarity rank, score, and trait floor prices |

### `nft_get_collection_data`

Use this when you need NFT collection floor price, volume, or holder data. Returns comprehensive collection stats from Reservoir for Ethereum and Base NFTs.
